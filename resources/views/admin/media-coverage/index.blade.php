...
<div class="admin-page-header">
    <div>
        <h1 class="admin-page-title">Kelola Liputan Media</h1>
        <p class="admin-page-subtitle">Manage logo & tautan berita media yang meliput Balong Hardi Sumedang</p>
    </div>
            ...
            <tbody>
                @forelse($mediaCoverages as $media)
                    <tr>
                        ...
                        <td>
                            @if($media->url)
                                <a href="{{ $media->url }}" target="_blank" class="url-cell" title="{{ $media->url }}">{{ $media->url }}</a>
                            @else
                                <span class="url-cell">-</span>
                            @endif
                        </td>
                        <td style="font-weight: 700;">{{ $media->order }}</td>
                        <td>
                            @if($media->is_active)
                        ...
                        <td>
                            <div class="action-group">
                                ...
                                <form action="{{ route('admin.media-coverage.destroy', $media) }}" method="POST" style="margin: 0;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus media ini?');">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-container">
                                <p style="color: #6B7280; font-size: 0.9rem; margin-bottom: 1.25rem;">Belum ada data liputan media yang ditambahkan.</p>
                                <button type="button" class="btn-bhs-primary" onclick="openModal('addModal')">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14M5 12h14"/></svg>
                                    Tambah Media
                                </button>
...
<div class="modal-overlay" id="addModal">
    <div class="modal-content">
        ...
        <form action="{{ route('admin.media-coverage.store') }}" method="POST" enctype="multipart/form-data" style="display:flex; flex-direction:column; flex:1; overflow:hidden;">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label for="name">Nama Media <span class="required">*</span></label>
                    <input type="text" id="name" name="name" class="input-control" placeholder="Contoh: Tribun Jabar" value="{{ old('name') }}" required>
                    @error('name')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="url">URL Berita (Opsional)</label>
                    <input type="url" id="url" name="url" class="input-control" placeholder="https://..." value="{{ old('url') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="logo" name="logo" class="input-control" accept="image/*">
            ...
            <div class="modal-actions">
                <button type="submit" class="btn-bhs-save-modal">Simpan Media</button>
...
<div class="modal-overlay" id="editModal">
    <div class="modal-content">
        <div class="modal-header">
            <h2>Edit Media</h2>
            <p>Perbarui rincian informasi media</p>
        </div>
        ...
        <form action="" method="POST" id="editForm" enctype="multipart/form-data" style="display:flex; flex-direction:column; flex:1; overflow:hidden;">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="url" id="edit_url" name="url" class="input-control" placeholder="https://...">
                    @error('url')<div class="form-error">{{ $message }}</div>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="edit_logo" name="logo" class="input-control" accept="image/*" onchange="previewImageMedia()">
                    <div class="form-hint">Kosongkan jika tidak ingin mengganti logo.</div>
                    @error('logo')<div class="form-error">{{ $message }}</div>@enderror
                </div>
